...
// Bulk update display orders for brands (admin only)
export const reorderBrands = withRequestContext({ protect: true, accessType: 'console' })(
  catchAsync(async context => {
    ...
    for (const item of reorderItems) {
      ...
      if (!mongoose.Types.ObjectId.isValid(item.id)) {
        throw new AppError(`Invalid brand id: ${item.id}`, 400);
      }
    }
    ...
    if (result.modifiedCount === 0 && result.matchedCount === 0) {
      throw new AppError('No brands were found to update', 404);
    }
