...
import { listTestimonials as listTestimonialsRepo } from '../../lib/firestore/collections';
...
export const listTestimonials =
  ...
  async ({ request }) => {
    ...
    const { items: testimonials, total } = await listTestimonialsRepo({ isActive, limit, page });

    return sendResponse(
      200,
      {
        testimonials: testimonials.map(t => ({ ...t, _id: t.id })),
        pagination: {
          total,
          page,
          limit,
