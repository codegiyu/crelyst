...
// Bulk update display orders for testimonials (admin only)
export const reorderTestimonials = withRequestContext({ protect: true, accessType: 'console' })(
  catchAsync(async context => {
    ...
    for (const item of reorderItems) {
      ...
      if (!mongoose.Types.ObjectId.isValid(item.id)) {
        throw new AppError(`Invalid testimonial id: ${item.id}`, 400);
      }
    }
    ...
    if (result.modifiedCount === 0 && result.matchedCount === 0) {
      throw new AppError('No testimonials were found to update', 404);
    }
