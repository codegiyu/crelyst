...
// Bulk update display orders for projects (admin only)
export const reorderProjects = withRequestContext({ protect: true, accessType: 'console' })(
  catchAsync(async context => {
    ...
    for (const item of reorderItems) {
      ...
      if (!mongoose.Types.ObjectId.isValid(item.id)) {
        throw new AppError(`Invalid project id: ${item.id}`, 400);
      }
    }
    ...
    if (result.modifiedCount === 0 && result.matchedCount === 0) {
      throw new AppError('No projects were found to update', 404);
    }
